...
import { FlexGap, Text, useTooltip } from "@cometswap/uikit";
...
type Props = {
  ...
  // Ratio applied to veCOMETwhen calculating iCOMET
  ratio?: number;
};
...
export function IveCometInfo({ snapshot, ratio = 1, ...props }: Props & SpaceProps) {
    ...
    <FlexGap flexDirection="column" gap="0.5rem" {...props}>
      {timeDisplay ? (
        <InfoItem
          label={t("Snapshot at")}
          value={timeDisplay}
          labelTooltip={t("The displayed iCOMETis calculated based on this snapshot time.")}
        />
      ) : null}
      <InfoItem
        label={t("Ratio")}
        value={`${ratio}x`}
        labelTooltip={t(
          "Your iCOMETis calculated by applying this ratio on the number of veCOMETat the snapshot time."
        )}
      />
    </FlexGap>
  );
