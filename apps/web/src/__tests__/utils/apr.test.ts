...
import { BIG_TEN, BIG_ZERO } from '@cometswap/utils/bigNumber'
...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { cometRewardsApr, lpRewardsApr } = getFarmApr(ChainId.BSC, null, null, null, null, 40)
    expect(cometRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`returns null when APR is infinite`, () => {
    const { cometRewardsApr, lpRewardsApr } = getFarmApr(ChainId.BSC, BIG_ZERO, BIG_ZERO, BIG_ZERO, '', 40)
    expect(cometRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`get the correct pool APR`, () => {
    const { cometRewardsApr, lpRewardsApr } = getFarmApr(
      ChainId.BSC,
      BIG_TEN,
      new BigNumber(1),
    ...
    expect(cometRewardsApr).toEqual(4204800)
    expect(lpRewardsApr).toEqual(0)
  })
