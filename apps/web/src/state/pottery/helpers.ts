...
export const transformPotteryPublicData = (publicData: SerializedPotteryPublicData): DeserializedPublicData => {
  const { totalPrize, totalLockveComet, totalSupply, totalLockedValue, maxTotalDeposit } = publicData

  return {
    ...publicData,
    totalPrize: totalPrize ? new BigNumber(totalPrize) : BIG_ZERO,
    totalLockveComet: totalLockveComet ? new BigNumber(totalLockveComet) : BIG_ZERO,
