import { MAX_VECOMET_LOCK_WEEKS, WEEK } from 'config/constants/Comet'
...
export const useMaxUnlockTime = (durationSeconds: number, prevUnlockTimestamp: number = 0) => {
  ...
  const maxUnlockTimestamp = useMemo(
    () => dayjs.unix(prevUnlockTimestamp ?? currentBlockTimestamp).add(MAX_VECOMET_LOCK_WEEKS, 'weeks'),
  ...
  return useMemo(() => {
    ...
    const leftWeeks = MAX_VECOMET_LOCK_WEEKS - (prevWeeks - currentWeeks)
...
export const useMaxUnlockWeeks = (weeksToLock: number, prevUnlockTimestamp: number = 0) => {
  ...
  return useMemo(() => {
    if (!prevUnlockTimestamp) return MAX_VECOMET_LOCK_WEEKS
