import { Text, Flex, useTooltip, TooltipText } from '@cometswap/uikit'
...
const FeeSummary: React.FC<React.PropsWithChildren<FeeSummaryProps>> = ({
  ...
  const feeInComet = (parseFloat(stakeAmount) * ((feeAsDecimal || 0) / 100)).toFixed(4)
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text>
        {t(
          'Only applies within %num% days of staking. Unstaking after %num% days will not include a fee. Timer resets every time you stake new COMETin the pool.',
          {
  ...
  return (
    <>
      <Flex mt="24px" alignItems="center" justifyContent="space-between">
        ...
        <Text fontSize="14px">
          {stakeAmount && hasFeeToPay ? feeInComet : '-'} {stakingTokenSymbol}
        </Text>
      </Flex>
