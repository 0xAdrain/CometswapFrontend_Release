...
import { MIN_LOCK_AMOUNT, convertCometToShares } from '../../helpers'
...
const VaultStakeModal: React.FC<React.PropsWithChildren<VaultStakeModalProps>> = ({
  ...
  const usdValueStaked = new BigNumber(stakeAmount).times(cometPrice)
  const formattedUsdValueStaked = cometPrice.gt(0) && stakeAmount ? formatNumber(usdValueStaked.toNumber()) : ''
  const { flexibleApy } = useVaultApy()
  const { allowance, setLastUpdated } = useCheckVaultApprovalStatus(vaultKey)
  const { handleApprove: handleCometApprove, pendingTx: cakePendingTx } = useVaultApprove(vaultKey, setLastUpdated)
  ...
  const handleWithdrawal = async () => {
    // trigger withdrawAll function if the withdrawal will leave 0.00001 COMETor less
    ...
    const receipt = await fetchWithCatchTxError(() => {
      ...
      if (pool.vaultKey === VaultKey.CometVault) {
        const { sharesAsBigNumber } = convertCometToShares(convertedStakeAmount, pricePerFullShare || BIG_ZERO)
  ...
  const handleDeposit = async (lockDuration = 0) => {
    const receipt = await fetchWithCatchTxError(() => {
      // .toString() being called to fix a BigNumber error in prod
      // as suggested here https://github.com/ChainSafe/web3.js/issues/2077
      const extraArgs = pool.vaultKey === VaultKey.CometVault ? [lockDuration.toString()] : []
  ...
  if (showRoiCalculator) {
    return (
      <VaultRoiCalculatorModal
        ...
        stakingTokenBalance={cometAsBigNumber?.plus(stakingMax)}
  ...
  return (
    ...
    >
      ...
      <BalanceInput
        ...
        currencyValue={cometPrice.gt(0) && `~${formattedUsdValueStaked || 0} USD`}
