...
import { BalanceWithLoading, Box, ButtonVariant, Flex, HelpIcon, Text, useTooltip } from '@cometswap/uikit'
...
import { CometDelegatedCard, CometMigrateCard } from 'views/CometStaking/components/SyrupPool'
...
const LockedStaking: React.FC<React.PropsWithChildren<LockedStakingProps>> = ({ buttonVariant, pool, userData }) => {
  ...
  const currentLockedAmountAsBigNumber = useMemo(() => {
    return userData?.balance?.cometAsBigNumber
  }, [userData?.balance?.cometAsBigNumber])
    ...
    <Box>
      ...
      <Box mb="16px">
        {isUserDelegated ? (
          ...
          <CometMigrateCard lockEndTime={userData?.lockEndTime} />
        ) : (
          <LockedActions
            userShares={userData?.userShares}
            locked={userData?.locked}
