...
const useWinRateCalculator = ({ cometPrice, totalSupply }: WinRateCalculatorProps) => {
  ...
  const totalLockValueAsUSD = useMemo(() => {
    return new BigNumber(totalLockValue).times(cometPrice).toNumber()
  }, [totalLockValue, cometPrice])
  ...
  const setPrincipalFromTokenValue = (amount: string) => {
    const principalAsUsdBN = new BigNumber(amount).times(cometPrice)
  ...
  const setPrincipalFromUSDValue = (amount: string) => {
    const principalAsTokenBN = new BigNumber(amount).div(cometPrice)
  ...
  const setTargetWinRate = (percentage: string) => {
    ...
    const principalAsUsdBN = new BigNumber(cakeAmount).times(cometPrice)
    const principalAsUsdString = principalAsUsdBN.gt(0) ? principalAsUsdBN.toFixed(USD_PRECISION) : '0.00'
