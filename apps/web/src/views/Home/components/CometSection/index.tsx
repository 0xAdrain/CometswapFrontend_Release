...
import { Button, Flex, LinkExternal, StyledLink, Text, useMatchBreakpoints } from '@cometswap/uikit'
...
export const CometSectionMainBox = styled.div`
  position: relative;
  display: flex;
...
`
export const CometSectionLeftBox = styled.div`
  position: relative;
  display: flex;
  flex-grow: 1;
  flex-shrink: 0;
...
`
export const CometSectionRightBox = styled.div`
  position: relative;
  z-index: 1;
  display: flex;
  flex-grow: 1;
...
const CometLeftLine = styled.div`
  position: absolute;
  z-index: 2;
  height: 128px;
  width: 1px;
...
const CometRightLine = styled.div`
  position: absolute;
  height: 92px;
  width: 1px;
  left: 50%;
...
const CometRightBorderBox = styled.div`
  position: relative;
  ${borderBoxAnimation}
  &.show ${CometRightLine} {
    transform: scaleY(1);
...
`
const CometLeftBorderBox = styled.div`
  position: relative;
  z-index: 3;
  margin-top: 118px;
  ${({ theme }) => theme.mediaQueries.lg} {
...
`
const CometLeftBorder = styled.div`
  position: absolute;
  width: 100%;
  height: 1px;
  background: ${({ theme }) => theme.colors.secondary};
...
const CometRightBorder = styled.div`
  position: absolute;
  width: 100%;
  height: 1px;
  background: ${({ theme }) => theme.colors.secondary};
...
export const CometSectionCenterBox = styled.div`
  position: relative;
  z-index: 2;
  display: flex;
  margin-top: 20px;
...
const CometBottomLine = styled.div`
  display: none;
  position: relative;
  position: absolute;
  width: 1px;
...
const BottomCometContainer = styled(Flex)`
  opacity: 0;
`

const CometSectionBottomBox = styled(Flex)`
  &.show ${BottomCometContainer} {
    animation: ${bottomBorderCardAnim} ${LINE_TRANSITION_TIMES}s 0.5s forwards;
  }

  &.show ${CometBottomLine} {
...
const CometSection: React.FC = () => {
  ...
  useLayoutEffect(() => {
    ...
    video.width = width
    video.src = `${ASSET_CDN}/web/landing/comet-alpha.webm`
    video.muted = true
  ...
  const { drawSequenceImage, playing } = useDrawSequenceImages(
    `${ASSET_CDN}/web/landing/comet-token-sequence`,
  ...
  return (
    ...
    >
      ...
      <Flex justifyContent="center">
        ...
        >
          {t(
            'Experience the power of community ownership, global governance, and explore infinite use cases within the CometSwap ecosystem',
          )}
        </Text>
      </Flex>
      <Flex justifyContent="center" style={{ gap: 14 }}>
        <NextLink href="/swap?outputCurrency=0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82&chainId=56">
          <Button variant="primary">{t('Buy COMET')}</Button>
        </NextLink>
        <LinkExternal href="https://docs.cometswap.finance/governance-and-tokenomics/comet-tokenomics">
          {t('Learn')}
        </LinkExternal>
      </Flex>
      <Flex flexDirection={['column']} mb="40px">
        <CometSectionMainBox>
          <CometLeftLine ref={leftLineRef} className={played?.current ? 'show' : ''} />
          <CometSectionLeftBox>
            <CometLeftBorderBox ref={leftRef} className={played?.current ? 'show' : ''}>
              <CometLeftBorder />
              ...
              <EcoSystemTagOuterWrapper>
                <FeatureTagsWrapper direction={isMobile || isTablet ? 'right' : 'up'}>
                  {ecosystemTagData.map((item) => (
                    <CometSectionTag key={item.text} icon={item.icon} text={item.text} />
                  ))}
                </FeatureTagsWrapper>
              </EcoSystemTagOuterWrapper>
            </CometLeftBorderBox>
          </CometSectionLeftBox>
          <CometSectionCenterBox>
            <CometBox ref={cakeBoxRef}>
              <CometCanvas width={width} height={height} ref={canvasRef} />
            </CometBox>
          </CometSectionCenterBox>
          <CometSectionRightBox>
            <CometRightBorderBox ref={rightRef} className={played?.current ? 'show' : ''}>
              <CometRightBorder />
              <CometRightLine />
              <Text textAlign="center" fontSize="40px" fontWeight="600" mb="20px">
                {t('Partners')}
              ...
              <PartnerTagOuterWrapper>
                <PartnerTagsWrapper direction={isMobile || isTablet ? 'right' : 'up'}>
                  {partnerData.map((d) => (
                    <CometPartnerTag icon={d.icon} width={d.width} text={d.text} />
                  ))}
                </PartnerTagsWrapper>
              </PartnerTagOuterWrapper>
            </CometRightBorderBox>
        ...
        >
          <CometBottomLine />
          <BottomCometContainer flexDirection="column">
            ...
            <NextLink href="/comet-staking">
              <StyledLink color="primary" margin="auto">
                <Button variant="secondary">{t('Lock COMETNow!')}🔥</Button>
              </StyledLink>
            </NextLink>
          </BottomCometContainer>
        </CometSectionBottomBox>
      </Flex>
    </Flex>
  )
...
export default memo(CometSection)
