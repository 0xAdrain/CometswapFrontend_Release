...
export const CometPartnerWrapper = styled.div<{ $bgWidth: number }>`
  display: flex;
  justify-content: center;
  align-items: center;
  height: 40px;
...
export const CometSectionTag: React.FC<{ icon: React.ReactElement; text: string }> = ({ icon, text }) => {
...
export const CometPartnerTag: React.FC<{
  ...
  const [bgWidth, setBgWidth] = useState(() => textRef?.current?.offsetWidth ?? 0)
  return (
    <CometPartnerWrapper $bgWidth={bgWidth + 58}>
