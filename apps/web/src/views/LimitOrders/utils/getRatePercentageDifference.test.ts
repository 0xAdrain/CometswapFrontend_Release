import { ERC20Token, Percent, Price } from '@cometswap/sdk'
...
describe('limitOrders/utils/getRatePercentageDifference', () => {
  describe('18 decimal tokens', () => {
    const marketPrice = new Price(COMET, BUSD, EIGHTEEN_DECIMALS, TEN) // 10 BUSD per 1 COMET
    it('returns correct positive percentage', () => {
      const price = new Price(COMET, BUSD, EIGHTEEN_DECIMALS, FIFTEEN) // 15 BUSD per 1 COMET
    ...
    it('returns correct negative percentage', () => {
      const price = new Price(COMET, BUSD, EIGHTEEN_DECIMALS, FIVE) // 5 BUSD per 1 COMET
      const rate = getRatePercentageDifference(marketPrice, price)
      const expectedRate = new Percent(-50, 100)
      expect(rate).toBeDefined()
      expect(expectedRate.equalTo(rate!)).toBe(true)
    ...
    it('returns correct equal percentage', () => {
      const price = new Price(COMET, BUSD, EIGHTEEN_DECIMALS, TEN) // 50 BUSD per 1 COMET
      const rate = getRatePercentageDifference(marketPrice, price)
