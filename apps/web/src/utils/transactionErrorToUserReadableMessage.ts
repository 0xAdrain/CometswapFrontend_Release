...
export function transactionErrorToUserReadableMessage(error: any, t: TranslateFunction) {
  ...
  switch (reason) {
    ...
    case 'CometRouter: INSUFFICIENT_OUTPUT_AMOUNT':
    case 'CometRouter: EXCESSIVE_INPUT_AMOUNT':
    case 'CometRouter: INSUFFICIENT_A_AMOUNT':
    case 'CometRouter: INSUFFICIENT_B_AMOUNT':
    ...
    case 'STF':
      return t(
        'This transaction will not succeed due to price movement. Try increasing your slippage tolerance. Note: fee on transfer and rebase tokens are incompatible with Cometswap V3.',
      )
    case 'TF':
      return t(
        'The output token cannot be transferred. There may be an issue with the output token. Note: fee on transfer and rebase tokens are incompatible with Cometswap V3.',
      )
    default:
